...
public class RESAudioClient {
    ...
    private IRtspSender audioRtspSender;
    ...
    public boolean prepare(RESConfig resConfig) {
        synchronized (syncOp) {
            ...
            prepareAudio();
            if (audioRtspSender != null) {
                audioRtspSender.onSetAudioSampleRate(resCoreParameters.audioRecoderSampleRate);
                softAudioCore.setAudioRtspSender(audioRtspSender);
            }
            return true;
        }
    }

    public void setAudioRtspSender(IRtspSender audioRtspSender) {
        this.audioRtspSender = audioRtspSender;
        if (softAudioCore != null) {
            softAudioCore.setAudioRtspSender(audioRtspSender);
        }
    }
